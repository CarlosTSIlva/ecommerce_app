...
void main() {
  testWidgets('Cancel logout, success', (WidgetTester tester) async {
  ...
  testWidgets('Confirm logout, failure', (WidgetTester tester) async {
    ...
    when(authRepository.authStateChanges).thenAnswer(
      (_) => Stream.value(
        const AppUser(uid: "123", email: "carlosteixeira.tc@hotmail.com"),
      ),
    );
  ...
  testWidgets('Confirm logout, loading state', (WidgetTester tester) async {
    final r = AuthRobot(tester);
    final authRepository = MockAuthRepository();

    when(authRepository.signOut).thenAnswer((_) => Future.delayed(
